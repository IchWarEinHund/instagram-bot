...
class Example(tk.Frame):
    ...
    def init_ui(self):
        self.title_label = tk.Label(self, text='liking by a location')
        ...
        #start button
        def startBot():
            messagebox.showinfo("Starting liking by a location:)", loginInsta.get() + " " + passwordInsta.get())
            ...
            session.set_user_interact(amount=3,
                                      ...
                                      percentage=100,
                                      media='Photo')
            session.like_by_locations([location.get()], amount=100)
            session.login()
