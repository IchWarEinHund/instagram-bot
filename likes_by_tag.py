...
class Example(tk.Frame):
    ...
    def init_ui(self):
        self.title_label = tk.Label(self, text='liking by a tag')
        ...
        #start button
        def startBot():
            messagebox.showinfo("Starting liking by a tag:)", loginInsta.get() + " " + passwordInsta.get())
            ...
            session.set_user_interact(amount=3,
                                      ...
                                      percentage=100,
                                      media='Photo')
            session.like_by_tags([hashtag1.get(), hashtag2.get()],
                                 amount=50,
                                 interact=True)
            session.login()
