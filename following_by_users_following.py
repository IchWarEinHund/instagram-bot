...
class Example(tk.Frame):
    ...
    def init_ui(self):
        self.title_label = tk.Label(self, text='following by a users following')
        ...
        #start button
        def startBot():
            messagebox.showinfo("Starting following by a users following)", loginInsta.get() + " " + passwordInsta.get())
            ...
            session.set_user_interact(amount=3,
                                      ...
                                      percentage=100,
                                      media='Photo')
            session.follow_user_following([friend1.get(), friend2.get(), friend3.get()],
                                          amount=50,
                                          interact=True)
            session.login()
